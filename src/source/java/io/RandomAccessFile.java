...
public class RandomAccessFile implements DataOutput, DataInput, Closeable {
    ...
    /**
     * 每次从文件中读取 一个字节 的内容，并以int型将读取的数据返回
     *
     * @return
     * @throws IOException
     */
     ...
     *
     * 一直读完位置，把字节数组填充完，或者到达文件末尾，或者抛出异常
     *
     * @param      b   the buffer into which the data is read.
    ...
    /**
     * 用于跳过指定字节长度的数据，传入的参数为一个long型数据，其表示要跳过的字节长度
     *
     * @param pos
     * @throws IOException
     */
    ...
    /**
     * 一次读取一行数据
     * 注意里面用的是StringBuffer，所以是线程安全的方法
     *
     * @return
     * @throws IOException
     */
    public final String readLine() throws IOException {
        ...
        // 通过循环不断读取，直到为-1或者遇到换行符 '\n'
        while (!eol) {
            //
            switch (c = read()) {
            case -1:
            case '\n':
                // 文件读完或者遇到换行符，下次循环就会跳出
                eol = true; // 置为true，下次循环就直接跳出
                break;
            case '\r':
                // win系统换行符是 \r\n
                eol = true;
    ...
    public final String readUTF() throws IOException {
        // 间接调用了DataInputStream的readUTF方法
    ...
    public final void writeBoolean(boolean v) throws IOException {
        // 从文件当前位置开始写，是一个字节吗？？？
        // 对的，看调用的writeByte方法，实际都是调用了write方法
        write(v ? 1 : 0);
        //written++;
    }
    ...
    public final void writeBytes(String s) throws IOException {
        // 当前位置写入一个字符串
        // 先把字符串转成字节数组，然后调用writeBytes方法写入
        int len = s.length();
        byte[] b = new byte[len];
