...
public abstract class AbstractQueuedSynchronizer
    ...
    static final class Node {
         ...
         * <p>
         * 表示对锁的共享模式：锁允许多个线程获得 NODE.SHARED
         */
        static final Node SHARED = new Node();
        /**
         * Marker to indicate a node is waiting in exclusive mode
         * <p>
         * 表示对锁的独占模式：锁只允许一个线程获得 NODE.EXCLUSIVE
         */
         ...
         * <p>
         * 后继节点的线程处于等待状态，而当前节点的线程如果释放了同步状态或被取消
         * 那么就会通知后续节点，让后继节点的线程能够运行
         */
        static final int SIGNAL = -1;
        /**
         * waitStatus value to indicate thread is waiting on condition
         * <p>
         * 节点在等待队列中，节点线程等待在condition上，不过当其他的线程对Condition调用了signal()方法后
         ...
         */
        volatile Thread thread; // 当前node节点包装的线程
         ...
         * mode.
         */
        Node nextWaiter;    // 表示当前节点对锁的模式，独占锁的话就是null，共享锁为Node()
    ...
    private void setHead(Node node) {
        head = node;
        // 头节点的线程指向为null，因为
        // 用exclusiveOwnerThread表示了当前获取锁的线程
        node.thread = null;
        node.prev = null;
    }
    ...
    private void doAcquireShared(int arg) {
        ...
        try {
            ...
            for (; ; ) {
                ...
                if (p == head) {    // 如果前一个节点是head头节点
                    int r = tryAcquireShared(arg);  // r代表还剩几个
                    if (r >= 0) {
                ...
                // 如果前一个节点不是头节点，就要判断这个节点是否可以park休眠
                // 只有前一个节点是SIGNAL状态的时候当前节点才可以去休眠
                if (shouldParkAfterFailedAcquire(p, node) &&
                        parkAndCheckInterrupt())
                    interrupted = true;
            }
    ...
    public final boolean hasQueuedPredecessors() {
        // 返回false的情景，表示当前线程没有前置节点，符合去获取锁的条件
        // 1. h == t, 那肯定符合条件，说明这个队列里就没有节点
        // 2. s = h.next != null, 说明头节点有后继节点，此时需要判断后继节点是不是当前节点
        ...
        Node h = head;
        Node s;
        // 如果只有一个节点，那么tail节点和这个节点是一个节点，即tail指向这个节点
        // (s = h.next) == null说明只有一个节点，即
        return h != t &&
     ...
     * <p>
     * 从AQS的后往前遍历
     * 如果遇到node节点，则说明node节点在同步队列
     * 如果遍历到head，head.prev为null，说明同步队列里没有node
     *
    ...
    final boolean transferForSignal(Node node) {
         ...
         *
         * 调用signal后，会把node的waitStatus状态变成0
         * 加入到条件队列的时候，会将status变成-2
         */
         ...
         */
        // 状态改成0后，把node加入到同步队列中，返回的是同步队列尾节点的前一个节点
        Node p = enq(node);
        int ws = p.waitStatus;
        // ws > 0说明是被取消了，只有CANCEL状态是 1，此时满足 ws > 0
        if (ws > 0 || !compareAndSetWaitStatus(p, ws, Node.SIGNAL))
            // ws > 0表示前一个节点的状态是 CANCELED
            // 只要前一个节点状态是CANCELED或者修改状态为SIGNAL失败，那么直接
            // 唤醒 node 节点的线程
    ...
    final int fullyRelease(Node node) {
        boolean failed = true;
        try {
            int savedState = getState();    // 获取当前AQS的state状态
            // 调用await会释放锁，把可重入的次数都减掉，相当于把state置0
            ...
            if (failed)
                // 走到这说明当前线程抛出异常了，所以failed才会为true
                // 不然要么是false，要么在上面就throw出异常了
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
        ...
        private Node addConditionWaiter() {
            ...
            if (t != null && t.waitStatus != Node.CONDITION) {
                // 从等待队列的头节点开始，跳过waitStatus为CANCELED状态的节点
                unlinkCancelledWaiters();
                t = lastWaiter;
            }
            Node node = new Node(Thread.currentThread(), Node.CONDITION);
            // 将新建的状态为CONDITION的节点添加到等待队列的尾部
            if (t == null)
                firstWaiter = node;
            else
                t.nextWaiter = node;
        ...
        private void doSignal(Node first) {
            ...
            // while里的条件表示如果signal线程失败，那么会自动的去signal等待队列中的下一个node
        }
        ...
        public final void await() throws InterruptedException {
            if (Thread.interrupted())   // 检测当前线程的中断标志位是否为true
                throw new InterruptedException();
            Node node = addConditionWaiter();
            int savedState = fullyRelease(node);    // 释放锁
            int interruptMode = 0;
            while (!isOnSyncQueue(node)) {
                // 如果不在同步队列，则使用park将当前线程阻塞，等待被唤醒
                // 不在同步队列，那说明就在等待队列
                LockSupport.park(this);
                // 要么是被唤醒，要么是被中断
                if ((interruptMode = checkInterruptWhileWaiting(node)) != 0)
                    // 只要是被中断唤醒的，那一定会break跳出while循环
                    break;
            }
            // 操作节点去申请锁
